...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model=None, sample=None, visual_hallucination=None, src_img_features=None,reduce=True):
        ...
        if net_output[1].encoder_out is not None:
            if net_output[1].src_img_features is not None:
                for each_layer_img in range(len(net_output[1].src_img_features)):
                    kl_divergence = self.kl_divergence_loss(net_output[1].hallucination_train.float().cpu(), net_output[1].src_img_features[each_layer_img].float().cpu(), net_output[1].src_img_features[each_layer_img].size(-1), net_output[1].hallucination_train.size(-1))
                    all_kl_divergence.append(kl_divergence)
                loss = loss.to(device) + 0.5*kl_divergence +0.5*all_kl_divergence[-2] + 0.1*all_kl_divergence[-3] + 0.05*all_kl_divergence[-4] + 0.01 *all_kl_divergence[-5] + 0.005 *all_kl_divergence[-6]
    ...
    def kl_divergence_loss(self, visual_hallucination, src_img_features, dim, dim_2, epsilon=1e-10):
        ...
        kl_div = F.kl_div(log_visual_hallucination_prob, src_img_features_prob, reduction='batchmean')
